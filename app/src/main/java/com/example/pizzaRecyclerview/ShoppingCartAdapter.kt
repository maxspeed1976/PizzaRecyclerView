...
class ShoppingCartAdapter(var context: Context, var cartItems: List<CartItem>) :
    ...
    class ViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bindItem(cartItem: CartItem) {
            ...
            //Picasso.get().load(cartItem.pizza.imageUrl).fit().into(itemView.product_image)
            Picasso.get().load(cartItem.pizza.imageUrl).into(itemView.product_image)




            itemView.product_name.text = cartItem.pizza.name

            itemView.product_price.text = "$${cartItem.pizza.price}"
